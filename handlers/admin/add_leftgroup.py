...
async def handle_add_to_group(message: types.Message):
...
async def handle_add_bot_to_group(message: types.Message):
    try:
        group_username = message.text.split("https://t.me/")[1]
        group = await bot.get_chat(f"@{group_username}")

        get_group_id = await db.get_group_on_status(group.id)
        await db.update_group_on_status(True, get_group_id['group_id'])
